# Code for thesis project about finding justifications for features of matching outcomes
...
def interpretVariable(x):
    if x > 0:
        ...
        print('-> where profile ' + str(p) + ' = ' + s)
# ------------------------------------------------------------------------------------------ #
# The principle of being a well-defined mechanism is split in two principles
# Return CNF encoding the property that every agent of type 0 is matched to at least one agent of type 1

def cnfLeftMechanism():
    cnf = []
    for p in matchsat.allProfiles():
        for i in matchsat.allIndices():
            cnf.append([matchsat.posLiteral(p,i,j) for j in matchsat.allIndices()])
    return cnf

#Return CNF encoding the property that every agent of type 1 is matched to at most one agent of type 0

def cnfRightMechanism():
    cnf = []
    for p in matchsat.allProfiles():
        for j in matchsat.allIndices():
            for i1 in matchsat.allIndices():
                for i2 in matchsat.indices(lambda i2 : i1 < i2):
                    cnf.append([matchsat.negLiteral(p,i1,j), matchsat.negLiteral(p,i2,j)])
    return cnf
...
def cnfFavorite():
    cnf = cnfLeftFavorite() + cnfRightFavorite()
    return cnf
...
#Return a cnf encoding the axiom of right-swap-stability
...
#Return a cnf encoding the axiom of swap-stability

def cnfSwapStable():
    cnf = cnfLeftSwapStable() + cnfRightSwapStable()
    return cnf

#Return a cnf encoding of the axiom no-bottoms-together
#If two agents have each other on the bottom of their preferences, they shouldn't be matched
...
#Returns True if a set of axioms(formulas) are a justification for the GOAL
#The goal is the negation of the feature
...
## RE encoding of basisSearch but for the function isJustification that takes axioms without names
## Given a corpus of axioms (a list of axioms with names) and a GOAL (the goal specifies the profile and the feature(negated))
## Returns the first subset of axioms that is a justification for the goal and saves the CNF with those axioms in a file under filename
...
# -----------------------------------------------------------------------------------
# Function to compare if two formulas have the same clauses
# Takes loooong
def compareCNF(cnf1,cnf2):
    n = 1
    for element in cnf1:
        if not element in cnf2:
            return False
        print('Element ', element, ' is in both formulas')
        print(n)
        n += 1
    return 'The formulas are the same'
...
# ----------------------------------------------------------------------------------------------------------
# Functions/things to generate results.


def plotSizes(xs,ys):
#    xs = np.array(['MECH','STA','TOPSTA','LSP','RSP','SP','GI','PI','LF','RF','FAV','LSW','RSW','SWSTA','NOBOT'])
#    ys = np.array([len(matchsat.cnfMechanism()), len(matchsat.cnfStable()),len(matchsat.cnfTopStable()),
#    len(matchsat.cnfLeftStrategyProof()),len(matchsat.cnfRightStrategyProof()),len(matchsat.cnfTwoWayStrategyProof()),
#    len(cnfGroupIndifferent()),len(matchsat.cnfLeftPeerIndifferent()), len(matchsat.cnfRightPeerIndifferent()), 
#    len(matchsat.cnfPeerIndifferent()),len(cnfLeftFavorite()),len(cnfRightFavorite()),len(cnfFavorite()),
#    len(cnfLeftSwapStable()),len(cnfRightSwapStable()),len(cnfSwapStable()),len(cnfNoBottoms())])
    plt.plot(xs, ys,
...
# Function to measure the time it takes to compute another function (that doesn't take any arguments)
...
# LIST OF AXIOMS
# --------------------------------------------------------------------
# stability = ['STA',matchsat.cnfStable()]
# top_stability = ['TOPSTA', matchsat.cnfTopStable()]
# left_strategyproofness = ['LSP', matchsat.cnfLeftStrategyProof()]
# right_strategproofness = ['RSP', matchsat.cnfRightStrategyProof()]
# strategyproofness = ['SP', matchsat.cnfTwoWayStrategyProof()]
# gi = ['GI', cnfGroupIndifferent()]
# pi = ['PI', matchsat.cnfPeerIndifferent()]
# left_favorite = ['LF', cnfLeftFavorite()]
# right_favorite = ['RF', cnfRightFavorite()]
# favorite = ['FAV', cnfFavorite()]
# left_swap_stable = ['LST', cnfLeftSwapStable()]
# right_swap_stable = ['RST', cnfRightSwapStable()]
# swap_stable = ['SS', cnfSwapStable()]
# no_bottoms = ['NOBOT', cnfNoBottoms()]
...
def generateMatchingOutcome(profile):
    cnf = []
    chosen_partners = []
    for i in matchsat.allIndices():
        matched = False
        while not matched:
            j = random.choice(matchsat.allIndices())
            if not j in chosen_partners:
                cnf.append([matchsat.posLiteral(profile,i,j)])
                matched = True
                chosen_partners += [j]
    return cnf
...
def plotNumFeaturesJustified(info,justifications):
    ...
    # plt.plot(profs, number_found,
    #          color="b",      # "r"=red, "b"=blue, "k"=black, "y"=yellow, ... (even shorter: c='r')
    #          linestyle="-",  # "--"=dashed, "-."=dash-dot, ":"=dotted, ... (even shorter: ls='-')
    #          marker="o",     # "o"=dot, "s"=square, "+"=cross, ...
    #          linewidth=0,    # (even shorter: lw=4)
    #          markersize=4,   # (even shorter: ms=10)
    #          )
    # plt.title('Number of features that could be justified for each profile')
    # plt.xlabel('Profiles')
...
# ------------------------------------------------------------
# Test for generateMatchingOutcome function
# ------------------------------------------------------------
# print('-------------------------------------')
# test = []
# for i in range(100):
#     p = drawProfile()
#     print('Profile: ', p)
#     x = generateMatchingOutcome(p)
#     print('Matching outcome: ', x)
#     b = isMatching(x,p)
#     test += [b]
#     print('isMatching?: ', b)
#     print('------------------------------------')
# print('Were all of them matchings? : ', all(test))
# ------------------------------------------------------------


# 100 runs number one

    info =  [[42784, [1, 1, 1]], [38995, [1, 1, 1]], [1339, [1, 1, 1]], [3210, [1, 1, 1]], [89, [1, 1, 1]], [16694, [1, 1, 1]], [35293, [1, 1, 1]], [39978, [1, 1, 1]], [13553, [1, 1, 1]], [23022, [1, 1, 1]], [30098, [1, 1, 1]], [33892, [1, 1, 1]], [4348, [1, 1, 1]], [16347, [1, 1, 1]], [21715, [1, 1, 1]], [42900, [1, 1, 1]], [44919, [1, 1, 1]], [46213, [1, 1, 1]], [23872, [1, 1, 1]], [45006, [1, 1, 1]], [35021, [1, 0, 0]], [5245, [1, 1, 1]], [2976, [1, 1, 1]], [37485, [1, 1, 1]], [4988, [1, 1, 1]], [14594, [1, 1, 1]], [17470, [1, 1, 1]], [241, [1, 1, 1]], [33304, [1, 1, 1]], [15821, [1, 1, 1]], [43514, [1, 1, 1]], [34641, [0, 0, 1]], [44777, [1, 1, 1]], [19796, [1, 1, 1]], [44902, [1, 1, 1]], [17015, [1, 1, 1]], [19674, [1, 1, 1]], [24647, [1, 1, 1]], [4880, [1, 1, 1]], [97, [1, 1, 1]], [23771, [1, 1, 1]], [24027, [1, 1, 1]], [21094, [1, 1, 1]], [35665, [1, 1, 1]], [45504, [1, 1, 1]], [23664, [1, 1, 1]
441], ['STA']], [[195443], ['TOPSTA']]]], [42900, [[[386101], ['TOPSTA']], [[386106], ['LSP', 'LSS']], [[386108], ['LSP', 'LSS']]]], [44919, [[[404273], ['LSP', 'TOPSTA']], [[404275], ['LSP', 'TOPSTA']], [[404280], ['TOPSTA']]]], [46213, [[[415919], ['LSP', 'TOPSTA']], [[415921], ['LSP', 'TOPSTA']], [[415926], ['TOPSTA']]]], [23872, [[[214851], ['LSP', 'LSS']], [[214852], ['TOPSTA']], [[214856], ['LSP', 'LSS']]]], [45006, [[[405055], ['STA']], [[405060], ['STA']], [[405062], ['TOPSTA']]]], [35021, [[[315192], ['LSP', 'TOPSTA', 'LSS']], [[315193],['N/A']], [[315197],['N/A']]]], [5245, [[[47206], ['TOPSTA']], [[47211], ['STA']], [[47213], ['STA']]]], [2976, [[[26785], ['TOPSTA']], [[26790], ['STA']], [[26792], ['STA']]]], [37485, [[[337368], ['STA']], [[337369], ['STA']], [[337373], ['TOPSTA']]]], [4988, [[[44895], ['TOPSTA']], [[44897], ['TOPSTA']], [[44899], ['TOPSTA']]]], [14594, [[[131347], ['LSP', 'TOPSTA']], [[131352], ['LSP', 'TOPSTA']], [[131354], ['TOPSTA']]]], [17470, [[[157233], ['STA']], [[157234], ['TOPSTA']], [[157238], ['STA']]]], [241, [[[2170], ['TOPSTA']], [[2175], ['STA']], [[2177], ['STA']]]], [33304, [[[299739], ['STA']], [[299741], ['STA']], [[299743], ['TOPSTA']]]], [15821, [[[142392], ['LSP', 'STA']], [[142394], ['LSP', 'STA']], [[142396], ['STA']]]], [43514, [[[391627], ['STA']], [[391632], ['STA']], [[391634], ['STA']]]], [34641, [[[311771],['N/A']], [[311775], ['N/A']], [[311776], ['STA']]]], [44777, [[[402995], ['LSP', 'TOPSTA']], [[402999], ['LSP', 'STA']], [[403000], ['LSP', 'STA']]]], [19796, [[[178165], ['STA']], [[178170], ['TOPSTA']], [[178172], ['STA']]]], [44902, [[[404120], ['TOPSTA']], [[404122], ['TOPSTA']], [[404127], ['TOPSTA']]]], [17015, [[[153138], ['LSP', 'TOPSTA', 'LSS']], [[153140], ['LSP', 'TOPSTA', 'LSS']], [[153142], ['LSP', 'TOPSTA']]]], [19674, [[[177067], ['TOPSTA']], [[177071], ['TOPSTA']], [[177075], ['TOPSTA']]]], [24647, [[[221826], ['LSS']], [[221828], ['LSS']], [[221830], ['LSP', 'TOPSTA']]]], [4880, [[[43921], ['TOPSTA', 'LSS']], [[43925], ['TOPSTA']], [[43929], ['TOPSTA', 'LSS']]]], [97, [[[874], ['TOPSTA']], [[879], ['STA']], [[881], ['STA']]]], [23771, [[[213942], ['TOPSTA', 'LSS']], [[213943], ['TOPSTA']], [[213947], ['TOPSTA', 'LSS']]]], [24027, [[[216245], ['TOPSTA']], [[216247], ['LSP', 'TOPSTA']], [[216252], ['LSP', 'TOPSTA']]]], [21094, [[[189847], ['TOPSTA']], [[189852], ['TOPSTA']], [[189854], ['TOPSTA']]]], [35665, [[[320988], ['STA']], [[320989], ['STA']], [[320993], ['STA']]]], [45504, [[[409537], ['TOPSTA']], [[409541], ['TOPSTA']], [[409545], ['TOPSTA']]]], [23664, [[[212977], ['TOPSTA']], [[212981], ['STA']], [[212985], ['STA']]]], [2713, [[[24418], ['TOPSTA']], [[24422], ['TOPSTA']], [[24426], ['TOPSTA']]]], [20122, [[[181100], ['STA']], [[181104], ['TOPSTA']], [[181105], ['STA']]]], [17463, [[[157169], ['TOPSTA']], [[157171], ['TOPSTA']], [[157176], ['TOPSTA']]]], [46279, [[[416514], ['STA']], [[416516], ['STA']], [[416518], ['TOPSTA']]]], [30502, [[[274520], ['STA']], [[274522], ['TOPSTA']], [[274527], ['STA']]]], [25540, [[[229863], ['LSS']], [[229865], ['LSS']], [[229867], ['TOPSTA']]]], [30235, [[[272116], ['TOPSTA']], [[272121], ['TOPSTA']], [[272123], ['TOPSTA']]]], [44632, [[[401689], ['STA']], [[401694], ['STA']], [[401696], ['TOPSTA']]]], [44333, [[[399000], ['LSP', 'LSS']], [[399002], ['LSP', 'LSS']], [[399004], ['STA']]]], [16632, [[[149690], ['LSP', 'TOPSTA']], [[149694], ['LSP', 'TOPSTA']], [[149695], ['TOPSTA']]]], [45047, [[[405426], ['LSS']], [[405427], ['LSS']], [[405431], ['TOPSTA']]]], [7088, [[[63794], ['LSP', 'TOPSTA', 'LSS']], [[63798],['N/A']], [[63799], ['N/A']]]], [34269, [[[308422], ['LSP', 'TOPSTA']], [[308426], ['STA']], [[308430], ['STA']]]], [2808, [[[25273], ['TOPSTA']], [[25277], ['LSP', 'TOPSTA']], [[25281], ['LSP', 'TOPSTA']]]], [32540, [[[292862], ['TOPSTA']], [[292864], ['LSP', 'TOPSTA']], [[292869], ['LSP', 'TOPSTA']]]], [45078, [[[405703], ['STA']], [[405707], ['STA']], [[405711], ['TOPSTA']]]], [41819, [[[376372], ['LSP', 'TOPSTA']], [[376376], ['TOPSTA']], [[376380], ['LSP', 'TOPSTA']]]], [16142, [[[145280], ['TOPSTA']], [[145282], ['LSP', 'TOPSTA']], [[145287], ['LSP', 'TOPSTA']]]], [10023, [[[90209], ['TOPSTA']], [[90213], ['LSP', 'TOPSTA']], [[90214], ['LSP', 'TOPSTA']]]], [16528, [[[148755], ['LSP', 'LSS']], [[148756], ['LSP', 'LSS']], [[148760], ['LSP', 'LSS']]]], [16702, [[[150320], ['TOPSTA']], [[150324], ['TOPSTA']], [[150325], ['TOPSTA']]]], [8793, [[[79139], ['TOPSTA']], [[79141], ['STA']], [[79146], ['STA']]]], [38813, [[[349319], ['STA']], [[349321], ['STA']], [[349326], ['TOPSTA']]]], [30637, [[[275735], ['STA']], [[275737], ['TOPSTA']], [[275742], ['STA']]]], [4984, [[[44859], ['TOPSTA']], [[44861], ['TOPSTA']], [[44863], ['TOPSTA']]]], [45805, [[[412248], ['LSP', 'TOPSTA']], [[412249], ['STA']], [[412253], ['STA']]]], [6691, [[[60220], ['TOPSTA']], [[60225], ['STA']], [[60227], ['STA']]]], [38082, [[[342739], ['STA']], [[342743], ['STA']], [[342747], ['STA']]]], [20966, [[[188696], ['LSP', 'TOPSTA', 'LSS']], [[188700], ['LSP', 'TOPSTA']], [[188701], ['NOBOT', 'LSS']]]], [24677, [[[222096], ['LSP', 'LSS']], [[222098], ['LSP', 'LSS']], [[222100], ['LSP', 'LSS']]]], [37258, [[[335325], ['STA']], [[335326], ['STA']], [[335330], ['TOPSTA']]]], [12029, [[[108264], ['TOPSTA']], [[108266], ['STA']], [[108268], ['STA']]]], [30949, [[[278543], ['TOPSTA']], [[278547], ['TOPSTA']], [[278548], ['TOPSTA']]]], [28256, [[[254306], ['LSP', 'TOPSTA']], [[254310], ['TOPSTA']], [[254311], ['LSP', 'TOPSTA']]]], [7642, [[[68781], ['TOPSTA']], [[68782], ['TOPSTA']], [[68786], ['TOPSTA']]]], [45225, [[[407028], ['STA']], [[407029], ['STA']], [[407033], ['TOPSTA']]]], [37546, [[[337915], ['TOPSTA', 'LSS']], [[337919], ['TOPSTA', 'LSS']], [[337923], ['TOPSTA']]]], [37180, [[[334623], ['STA']], [[334625], ['STA']], [[334627], ['TOPSTA']]]], [4480, [[[40323], ['TOPSTA']], [[40325], ['TOPSTA']], [[40327], ['TOPSTA']]]], [9092, [[[81830], ['TOPSTA']], [[81834], ['TOPSTA', 'LSS']], [[81835], ['TOPSTA', 'LSS']]]], [17964, [[[161678], ['LSP', 'TOPSTA']], [[161682], ['LSP', 'TOPSTA']], [[161683], ['TOPSTA']]]], [15816, [[[142345], ['TOPSTA']], [[142349], ['STA']], [[142353], ['STA']]]], [25419, [[[228773], ['TOPSTA']], [[228775], ['TOPSTA']], [[228780], ['TOPSTA']]]], [32226, [[[290036], ['LSP', 'TOPSTA']], [[290040], ['LSP', 'TOPSTA']], [[290041], ['TOPSTA']]]], [16700, [[[150302], ['TOPSTA']], [[150306], ['TOPSTA']], [[150307], ['TOPSTA']]]], [14074, [[[126669], ['TOPSTA']], [[126671], ['TOPSTA']], [[126673], ['TOPSTA']]]], [24097, [[[216874], ['LSP', 'NOBOT', 'LSS']], [[216878], ['LSP', 'TOPSTA', 'LSS']], [[216882], ['STA']]]], [6159, [[[55434], ['LSP', 'TOPSTA']], [[55435], ['LSP', 'TOPSTA']], [[55439], ['TOPSTA']]]], [1977, [[[17795], ['TOPSTA']], [[17799], ['STA']], [[17800], ['STA']]]], [12771, [[[114941], ['LSP', 'LSS']], [[114945], ['LSP', 'LSS']], [[114946], ['TOPSTA']]]], [36094, [[[324849], ['TOPSTA']], [[324851], ['TOPSTA']], [[324853], ['TOPSTA']]]], [14129, [[[127164], ['TOPSTA']], [[127165], ['TOPSTA']], [[127169], ['TOPSTA']]]], [26537, [[[238835], ['STA']], [[238837], ['TOPSTA']], [[238842], ['STA']]]], [46078, [[[414704], ['STA']], [[414708], ['STA']], [[414709], ['STA']]]]]
